...
ここでは中心に 1 粒子、半径 $r$ の円周上に互いに $120^\circ$ 離れて配置された 3 粒子から成る 4 体系を考えます。すべての粒子間には一般の $(p,q)$ 型レナード–ジョーンズ (LJ) ポテンシャル

$$
V(d) = 4 \left[ \left( \frac{1}{d} \right)^{p} - \left( \frac{1}{d} \right)^q \right]\qquad (p>q>0)
$$

が作用するとします ($\epsilon = \sigma = 1$ の規格化単位系)。従来の $(6,12)$ 形は $(p,q) = (12,6)$ に対応します。
...
平衡半径 $r^*$ は $U'(r) = 0$ を満たす $r>0$ で決まります。一般の $(p,q)$-LJ ポテンシャルの導関数は

$$
V'(d) = \frac{\mathrm{d}V}{\mathrm{d}d} = -4p\, d^{-(p+1)} + 4q\, d^{-(q+1)}
$$
...
と書けます。$(\sqrt{3}\,r)^{-n} = 3^{-n/2} r^{-n}$ を用いて整理すると

$$
U'(r) = 12\left[-p\left(1+3^{-p/2}\right) r^{-(p+1)} + q\left(1+3^{-q/2}\right) r^{-(q+1)}\right].
$$

これをゼロとおくと

$$
q\left(1+3^{-q/2}\right) r^{-(q+1)} = p\left(1+3^{-p/2}\right) r^{-(p+1)}
$$

すなわち

$$
r^{p-q} = \frac{p\left(1+3^{-p/2}\right)}{q\left(1+3^{-q/2}\right)}.
$$

したがって平衡半径は

$$
r^{*} = \left[\frac{p\left(1+3^{-p/2}\right)}{q\left(1+3^{-q/2}\right)}\right]^{\!1/(p-q)}.
$$

$(p,q) = (12,6)$ を代入すると

$$
r^{6} = \frac{2\left(1 + 3^{-6}\right)}{1 + 3^{-3}} = \frac{365}{189}, \qquad
r^{*} = \left(\frac{365}{189}\right)^{1/6},
$$

となり、数値的には $r^{*} \approx 1.116$（6 乗した値が約 1.931）です。

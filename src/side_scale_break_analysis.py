...
def measure_breaks(
    ...
    X, V = initialize_state(side_scale, vb, z0)
    ...
    for step in range(nsteps):
        X, V = lj4_3d.step_yoshida4(X, V, dt)
...
def plot_results(
    ...
    plt.title("Time until structure break vs side_scale")
    plt.xlim(0.8, 1.6)
    plt.legend()
    plt.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument(
        "--plot-out",
        type=Path,
        default=Path("results/side_scale_break.png"),
        help="where to store the time-to-break plot",
    )
    parser.add_argument(
        "--csv-out",
        type=Path,
        default=Path("results/side_scale_break.csv"),
    ...
    if not args.no_plot:
        plot_results(results, args.T, thresholds, args.plot_out)
    if not args.no_csv:
        save_csv(results, args.csv_out, args.T, thresholds)
